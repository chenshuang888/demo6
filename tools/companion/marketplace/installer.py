"""installer —— 解 .mpkg + 推 app 到 ESP32 + 落 plugin/ 到 tools/plugins/。
...
设计：
  ...
  - 解 .mpkg 后：
      ...
      * plugin/ 部分写到 tools/plugins/<slug>/，并在 .marketplace_meta/<slug>.json 留清单
...
def install_plugin_locally(parsed: ParsedMpkg, slug: str) -> tuple[Optional[str], list[str]]:
    """把 plugin/ 解到 tools/plugins/<slug>/，返回 (plugin_dir_name, 文件清单)。

    文件清单是相对 tools/ 的路径，便于卸载时定位。
    没有 plugin 时返回 (None, [])。
    """
    if not parsed.plugin_files:
        return None, []

    plugin_root = _plugins_dir() / slug
    ...
    for full_name, data in parsed.plugin_files.items():
        ...
        written.append(str(target.relative_to(_tools_dir())).replace("\\", "/"))

    return slug, written


def uninstall_plugin_locally(slug: str) -> None:
    """按 registry 记录删 tools/plugins/<slug>/。
    ...
    if plugin_dir_name:
        d = _plugins_dir() / plugin_dir_name
        if d.exists():
            # 自下而上删空目录
