"""Marketplace 页：浏览市场 + 一键安装/卸载/更新。

数据流：
  - 拉列表/详情：marketplace HTTP API（同步 requests，丢工作线程跑）
  - 安装：下载 .mpkg → 解析 → 写临时 pack_dir → bus("upload:request") 让 UploadProvider 推到设备
  - 卸载：bus("upload:request" delete) + 删 tools/plugins/<slug>/
  - 进度复用 upload:* 事件（与 UploadPage 一致）
"""
...
class MarketplacePage(ctk.CTkFrame):
    ...
    def _build(self) -> None:
        ...
        ctk.CTkLabel(top, text="市场地址", text_color=COLOR_MUTED, width=80, anchor="w") \
            .grid(row=0, column=0, padx=(14, 4), pady=10)
        self._url_var = ctk.StringVar(value=marketplace.config.get_base_url())
        self._url_entry = ctk.CTkEntry(top, textvariable=self._url_var)
        self._url_entry.grid(row=0, column=1, sticky="ew", padx=4, pady=10)
        ctk.CTkButton(top, text="保存", width=70, command=self._save_url) \
            .grid(row=0, column=2, padx=4, pady=10)
        ctk.CTkButton(top, text="刷新", width=70, fg_color=COLOR_ACCENT2,
    ...
    def _save_url(self) -> None:
        ...
        if not url:
            return
        marketplace.config.set_base_url(url)
        self._client = MarketplaceClient()
        self._set_status(f"已切换到 {url}", COLOR_OK)
        self._refresh_all()
    ...
    def _fetch_packages_worker(self) -> None:
        ...
        self.after(0, lambda: self._set_status(
            f"市场共 {total} 个包，本地已装 {len(self._installed)} 个", COLOR_OK))
    ...
    def _render_items(self, items: list[PackageCard]) -> None:
        ...
        if not items and not orphan_slugs:
            empty = ctk.CTkLabel(self._scroll, text="（市场为空）",
    ...
    def _render_card(self, row: int, pkg: PackageCard, *, orphan: bool) -> None:
        ...
        ctk.CTkLabel(title_row, text=f"  v{latest}", text_color=COLOR_MUTED, anchor="w") \
            .pack(side="left")
        if pkg.needs_plugin:
            ctk.CTkLabel(title_row, text="  ⚙ PC", text_color=COLOR_WARN, anchor="w") \
                .pack(side="left")
        if is_outdated:
    ...
    def _render_orphan(self, row: int, slug: str) -> None:
        ...
        ctk.CTkLabel(card, text=f"{slug} (孤儿，市场已下架)",
                       text_color=COLOR_TEXT, anchor="w",
                       font=ctk.CTkFont(size=14, weight="bold")) \
            .grid(row=0, column=1, sticky="ew", padx=4, pady=(12, 0))
        ctk.CTkLabel(card, text=f"本地版本 v{info.get('version', '?')}",
                       text_color=COLOR_MUTED, anchor="w") \
    ...
    def _on_install(self, slug: str, version: str) -> None:
        ...
        if not self._connected:
            messagebox.showwarning("未连接", "请先在 Home 页等设备连上")
            return

        # PC 插件二次确认
        # （我们这里下载后才知道是否含 plugin，所以提示语稍微泛一些）
        ok = messagebox.askokcancel(
            "确认安装",
            f"将下载并安装 {slug} v{version} 到设备。\n"
            f"如果该包带 PC 插件，会写入 tools/plugins/{slug}/ 并需要重启 companion 生效。\n\n"
            "继续？"
        )
        if not ok:
            return
    ...
    def _install_worker(self, slug: str, version: str) -> None:
        ...
        try:
            ...
            if parsed.has_plugin:
                self.after(0, lambda: self._set_status(f"安装 PC 插件到 tools/plugins/{slug}/…", COLOR_ACCENT))
            ...
            fut.result(timeout=300)  # 等上传完成；UploadProvider 会发 begin/progress/end
            ...
            self.after(0, lambda: self._set_status(
                ...
                + ("（含 PC 插件，请重启 companion 启用）" if parsed.has_plugin else ""),
                COLOR_OK))
    ...
    def _on_uninstall(self, slug: str) -> None:
        ...
        if not messagebox.askokcancel(
            ...
            f"并清理 tools/plugins/{slug}/。\n\n继续？"
        ):
            return
        if not self._connected:
            messagebox.showwarning("未连接", "请先连接设备再卸载")
            return
    ...
    def _uninstall_worker(self, slug: str) -> None:
        try:
            ...
            self.after(0, lambda: self._set_status(
                f"✅ {slug} 已卸载（PC 插件改动需重启 companion 生效）", COLOR_OK))
            self.after(0, self._refresh_all)
        except Exception as e:
